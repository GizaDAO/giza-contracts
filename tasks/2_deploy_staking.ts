...
task("deploy_staking", "Deploy Staking")
    ...
    const bondingCalculator = await BondingCalculator.deploy(
        initAddresses.GIZA, { gasLimit: 5e6 } // Gas estimation may fail
    );
    ...
    // Deploy sgiza
    const SGIZA = await ethers.getContractFactory('StakedToken');
    const sgiza = await SGIZA.deploy();
    await sgiza.deployed()
    console.log("SGIZA: " + sgiza.address);
    addressFile.set("SGIZA", sgiza.address);
