...
task("exec_staking", "Execute Staking")
    ...
    // Initialize sgiza and set the index
    await sgiza.initialize(staking.address);
    console.log("sgiza initialize success")
    await sgiza.setIndex(STAKING_CONFIG.INITIAL_INDEX, { gasLimit: 5e6 });
    console.log("sgiza setIndex success")
    ...
    // 0 2 4 5 8 9 
    // RESERVEDEPOSITOR,RESERVETOKEN,LIQUIDITYDEPOSITOR,LIQUIDITYTOKEN,REWARDMANAGER,SGIZA
    // 1 3 6 7
    ...
    // queue and toggle SGIZA
    await treasury.queue('9', sgiza.address, { gasLimit: 5e6 });
    ...
    await treasury.toggle('9', sgiza.address, ZERO_ADDRESS, { gasLimit: 5e6 });
    console.log("treasury toggle 9 success")

    // initialize circulating supply
    await circulatingSupply.initialize(initAddresses.GIZA, { gasLimit: 5e6 })
