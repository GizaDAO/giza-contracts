...
contract Treasury is Ownable {
    ...
    address public sGIZA;
    uint public sGIZAQueue; // Delays change to sGIZA address
    ...
    if ( _GIZADAI != address(0) ) {
        isLiquidityToken[ _GIZADAI ] = true;
        liquidityTokens.push( _GIZADAI );
    }
    ...
    /**
        @notice allow approved address to deposit an asset for GIZA
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint GIZA needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( GIZA ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn GIZA for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOfToken( _token, _amount );
        IGIZAERC20( GIZA ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sGIZA ).balanceOf( msg.sender ); // Can only borrow against sGIZA held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with GIZA
        @param _amount uint
     */
    function repayDebtWithGIZA( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IGIZAERC20( GIZA ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, GIZA, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( GIZA ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( GIZA ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns GIZA valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOfToken( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match GIZA decimals
            value_ = _amount.mul( 10 ** IERC20( GIZA ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SGIZA ) { // 9
            sGIZAQueue = block.number.add( blocksNeededForQueue );
        } else return false;
